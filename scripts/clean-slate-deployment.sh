...
aws logs delete-log-group --region $AWS_REGION \
  --log-group-name /aws/eks/wiz-security-demo-cluster/cluster 2>/dev/null || echo "Log group already deleted"

echo ""
echo "[Step 5/7] Resource names updated in code..."
echo "  - EKS cluster: wiz_cluster_eks → wiz-security-demo-cluster"
...
echo "   - EKS cluster (with new name: wiz-security-demo-cluster)"
echo "   - EC2 + MongoDB"
